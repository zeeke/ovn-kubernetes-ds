...
var _ = ginkgo.Describe("e2e ingress traffic validation", func() {
	...
	ginkgo.Context("Validating ingress traffic to manually added node IPs", func() {
		ginkgo.BeforeEach(func() {
			...
			// the client uses the netexec command from the agnhost image, which is able to receive commands for poking other
			// addresses.
			networkArgs := []string{"--network", "kind"}
			if externalContainerNetwork != "kind" {
				networkArgs = append(networkArgs, "--network", externalContainerNetwork)
			}
			createClusterExternalContainer(clientContainerName, agnhostImage, append(networkArgs, "-P"), []string{"netexec", "--http-port=80"})
		...
		ginkgo.AfterEach(func() {
			...
			ioutil.WriteFile("pause.txt", []byte("wait"), 0644)
			gomega.Eventually(func() bool {
				_, err := ioutil.ReadFile("pause.txt")
				return err != nil
			}, 1*time.Hour, 5*time.Second).Should(gomega.BeTrue())
